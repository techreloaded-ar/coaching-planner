 ...
 *
 * Verifica:
 * - Fallimento con messaggio chiaro quando SITEGROUND_DATABASE_URL manca
...
describe("extractDatabase", () => {
  ...
  it("dovrebbe gestire database con query string", () => {
    expect(
      extractDatabase("postgresql://u:p@h:5432/db?sslmode=require")
    ).toBe("db");
  });
  ...
  it("dovrebbe restituire il placeholder per database mancante", () => {
    expect(extractDatabase("postgresql://u:p@h:5432")).toBe("[database sconosciuto]");
  });
});
...
describe("validateConnectionString", () => {
  it("dovrebbe fallire quando la variabile è undefined", () => {
    const result = validateConnectionString(undefined);
    expect(result.valid).toBe(false);
    if (!result.valid) {
      expect(result.message).toContain("SITEGROUND_DATABASE_URL");
      expect(result.message).toContain("non è configurata");
    }
  });
...
describe("hasTlsFlag", () => {
  ...
  it("dovrebbe riconoscere sslmode=verify-full", () => {
    expect(hasTlsFlag("postgresql://u:p@h:5432/db?sslmode=verify-full")).toBe(true);
  });

  it("dovrebbe riconoscere sslmode=verify-ca", () => {
    expect(hasTlsFlag("postgresql://u:p@h:5432/db?sslmode=verify-ca")).toBe(true);
  });
...
describe("runDiagnostics — casi di errore", () => {
  ...
  it("dovrebbe uscire con errore quando la variabile è undefined", () => {
  ...
  it("dovrebbe stampare un messaggio di errore (non segreti) su stderr", () => {
    ...
    expect(errorOutput).toContain("SITEGROUND_DATABASE_URL");
    expect(errorOutput).toContain("non è configurata");
  });
});
