 ...
 *
 * Uso:
 *   npx tsx scripts/siteground-connectivity-check.ts [connection-string]
 *
 * Requisiti:
 *   - Stringa di connessione passata come argomento CLI, oppure variabile
 *     d'ambiente DATABASE_URL configurata
 *   - Formato atteso: postgresql://<utente>:<password>@<host>:5432/<database>?sslmode=require
 *
	...
	if (!url || url.trim() === "") {
		return {
			...
			message:
				...
				"  Passa la stringa come argomento CLI oppure imposta DATABASE_URL nell'ambiente.\n" +
				'  Esempio: npx tsx scripts/siteground-connectivity-check.ts "postgresql://utente:password@host:5432/db?sslmode=require"',
		};
	}
...
): ReturnType<typeof runConnectivityCheck> | never {
...
export async function runConnectivityCheck(connectionString: string) {
	...
	try {
		...
		if (
			...
			console.log(
				"   - Il certificato del server non è riconosciuto dalla CA di sistema",
			);
			console.log("   - sslmode=require non è accettato dal server");
			console.log(
				"   - Il server richiede sslmode=verify-full o una CA specifica",
			);
			console.log(
				"   Azione: verificare con SiteGround se è richiesto un certificato CA personalizzato",
			);
		} else if (
			...
			console.log(
				"🔍 Host non risolvibile. Verificare che l'hostname sia corretto e raggiungibile.",
			);
		} else if (errorMsg.includes("timeout") || errorMsg.includes("ETIMEDOUT")) {
			console.log(
				"🔍 Timeout di connessione. Verificare che l'host e la porta siano accessibili dalla rete corrente.",
			);
		} else if (
			...
			console.log(
				"🔍 Errore di autenticazione. Verificare username e password.",
			);
		}
		console.log("────────────────────────────────────────────────────────\n");
		...
		try {
			...
			/* il client potrebbe non essere connesso */
		}
	...
	try {
		...
		const pgVersion =
			versionStr.match(/PostgreSQL ([\d.]+)/)?.[1] ?? "[non rilevata]";
		...
		const tzValue =
			tzRes.rows[0]?.Timezone ??
			tzRes.rows[0]?.timezone ??
			Object.values(tzRes.rows[0] ?? {})[0] ??
			"non disponibile";
    ...
    `);
		...
		if (pgMajor >= minMajor) {
			console.log(
				`✅ PostgreSQL ${pgVersion} è compatibile con Prisma 7.x (richiede ≥ ${minMajor}.0)`,
			);
		} else if (pgMajor >= 9 && pgMinor >= 4) {
			console.log(
				`⚠️  PostgreSQL ${pgVersion} è tecnicamente compatibile ma precedente alla versione raccomandata (≥ ${minMajor}.0).`,
			);
			console.log(
				"   Prisma 7.x con pg-adapter potrebbe funzionare ma non è garantito.",
			);
		} else {
			console.log(
				`❌ PostgreSQL ${pgVersion} NON è compatibile con Prisma 7.x (richiede ≥ 9.4).`,
			);
		}
		...
		try {
			...
			/* il client potrebbe essere già chiuso */
		}
	}
}
...
if (
	process.argv[1]?.endsWith("siteground-connectivity-check.ts") ||
	process.argv[1]?.endsWith("siteground-connectivity-check.js")
) {
	runDiagnostics(process.argv[2] ?? process.env.DATABASE_URL);
}
