...
Dove comunicare l'errore dipende da come l'action risponde, e la distinzione è importante perché non è simmetrica. Le action che tornano uno stato al chiamante — come l'eliminazione di un'offerta, che restituisce `{ errore }` — mostrano il messaggio dentro il modale, che resta aperto con il pulsante di nuovo abilitato. Le action che comunicano invece con `redirect()` non restituiscono nulla: Next **rigetta** la promise dell'action al posto di risolverla, quindi qualunque codice scritto dopo il loro `await` non viene mai eseguito e non esiste alcun esito da leggere lato client. Per quelle — invalidazione utente, eliminazione scaglione — l'attesa resta sul pulsante fino alla navigazione, che smonta il modale, e l'errore compare sul banner di pagina alimentato dai parametri dell'URL (`?errore=…`). Non si tenta di ricostruire l'esito lato client: sarebbe codice morto.
