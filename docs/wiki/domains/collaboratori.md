...
sources:
    ...
    - path: prisma/migrations/20260728135453_aggiungi_cognome_utente/migration.sql
      role: coordinated-data-migration
...
review:
    ...
    evidence_hash: sha256:c306f3a57110f7a2734371890e85ff927b28f9b2c388b1afc2b397bacd8412dd
...
1. `creaUtente` in `src/app/(back-office)/anagrafiche/utenti/actions.ts` è l'unico percorso di censimento: quando l'amministratore seleziona il ruolo Collaboratore nel censimento, la transazione crea `Utente` (ruolo `AMMINISTRATORE` o `COLLABORATORE` secondo i checkbox selezionati, `nome`/`cognome` distinti dall'anagrafica) e, in aggiunta, `Collaboratore` con `attivo: true` e gli stessi `nome`/`cognome`. Non riusa mai un utente esistente: un'email già presente produce sempre l'esito di duplicato senza scritture.
2. `aggiornaUtente` nello stesso file, in modifica, legge utente e profilo fuori transazione per decidere se i campi di profilo condizionati (partita IVA, tariffa) sono richiesti (`validaModificaUtente` con `profiloPresente`; il cognome è invece sempre richiesto in Anagrafica, indipendentemente dal profilo), poi nella transazione aggiorna sempre `Utente.nome`/`cognome` distinti (oltre a email/ruolo) e applica sul profilo esattamente una transizione: crea (`attivo: true`, con gli stessi `nome`/`cognome`) se il ruolo Collaboratore è aggiunto senza profilo preesistente; riattiva (`attivo: true`, `nome`, `cognome`) se il profilo esiste disattivato; disattiva (`attivo: false`, `nome`, `cognome`) se il ruolo Collaboratore è tolto e il profilo era attivo; se lo stato richiesto coincide con quello attuale e il profilo esiste già, sincronizza comunque `Collaboratore.nome`/`cognome` con l'anagrafica corrente (nessuna transizione di stato). La protezione dell'ultimo amministratore attivo si applica al ruolo derivato dai checkbox esattamente come prima di US-046. Rivalida l'anagrafica utenti sempre e quella collaboratori ogni volta che il profilo è stato toccato o risincronizzato.
3. `aggiornaCollaboratore` in `src/app/(back-office)/anagrafiche/collaboratori/actions.ts` — unica action esposta da questo modulo da US-047 — aggiorna nella stessa transazione profilo (nome, cognome, partita IVA, tariffa) e propaga `Utente.nome`/`Utente.cognome` distinti (non più una stringa composta da US-048), senza mai leggere né scrivere `Utente.email`, senza cambiare `attivo` o `ruolo`. Un valore `email` eventualmente presente nel FormData viene ignorato, non validato: la validazione email non esiste più in questo modulo (`RE_EMAIL` e il campo `email` sono stati rimossi da `valida-collaboratore.ts`). Censimento (`creaCollaboratore`) e cambio di stato (`cambiaStatoCollaboratore`, con l'adapter `cambia-stato-action.ts`) sono stati rimossi da US-047: l'UI dell'anagrafica collaboratori non offre più un percorso per crearli o disattivarli/riattivarli — `/anagrafiche/collaboratori/nuovo` reindirizza l'amministratore a `/anagrafiche/utenti`, e l'elenco/la modifica mostrano lo stato attivo/disattivato in sola lettura.
