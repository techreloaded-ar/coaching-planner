...
sources:
    ...
    - path: tests/unit/bootstrap-amministratore-iniziale.test.ts
      role: verification
review:
    content_hash: sha256:7e608bde3c337803145aaac782d98a16dfe198e231a2944215c1b828b7fabb19
    evidence_revision: 471a818f5645f5e5829c2c8e5fbe0f71f48597ae
    evidence_hash: sha256:ea0d54cc7900aa781bb9fa408eb311466bb0e230566496159d4e2804ac772957
...
Sia in creazione (`creaUtente`) sia in modifica (`aggiornaUtente`), il form usa lo stesso fieldset a checkbox indipendenti (`ruoloAmministratore`, `ruoloCollaboratore`, entrambi opzionali ma non simultaneamente assenti), validati rispettivamente da `validaCensimentoUtente` e `validaModificaUtente`: nessun ruolo selezionato è rifiutato con l'errore "Seleziona almeno un ruolo" senza alcuna scrittura. Da US-048, nome e cognome sono campi anagrafici distinti e sempre richiesti nella sezione Anagrafica, indipendentemente dai ruoli selezionati: un cognome mancante è rifiutato con "Il cognome è obbligatorio" anche per un utente solo-amministratore (vedi la decisione [Cognome nella sezione anagrafica dell'utente](/decisions/cognome-anagrafica-utente.md)). Quando il ruolo Collaboratore è selezionato, restano invece condizionati partita IVA e tariffa giornaliera, con gli stessi controlli e messaggi dell'anagrafica collaboratori (helper condivisi `validaCampoPartitaIva`/`validaCampoTariffaGiornaliera` di `src/domain/anagrafiche/valida-collaboratore.ts`) — ma solo quando il profilo non esiste ancora: `validaModificaUtente` riceve `profiloPresente` e non richiede mai quei campi se un profilo (attivo o disattivato) è già collegato all'utente, coerentemente con l'assenza della sezione profilo nel form in quel caso.
...
8. `creaUtente` normalizza nome, cognome ed email, valida con `validaCensimentoUtente`, impedisce duplicati anche traducendo il vincolo Prisma `P2002` (l'email già esistente è sempre un errore di duplicato, non viene mai riusato un utente esistente) e crea in un'unica transazione un utente con `nome`/`cognome` distinti e `attivo = true` per default di schema; assegna `ruolo: "AMMINISTRATORE"` se il ruolo Amministratore è selezionato, altrimenti `"COLLABORATORE"`. Se il ruolo Collaboratore è selezionato, la stessa transazione crea anche `Collaboratore` con gli stessi `nome`/`cognome` dell'anagrafica (oltre a partita IVA, tariffa normalizzata, `attivo: true`) — da US-048 `Utente.nome` non compone più la stringa `"{nome} {cognome}"`, la colonna `Utente.cognome` è distinta e obbligatoria per ogni combinazione di ruoli; l'elenco utenti deriva quindi due badge di ruolo indipendenti (Amministratore dall'enum, Collaboratore dall'enum oppure dalla sola presenza del profilo). `aggiornaUtente`, in modifica, legge fuori transazione utente e profilo, valida con `validaModificaUtente` e calcola il nuovo ruolo dai checkbox; nella transazione aggiorna sempre `nome`/`cognome`/email/ruolo distinti e applica sul profilo esattamente una transizione: crea (con gli stessi `nome`/`cognome`) se il ruolo Collaboratore è aggiunto senza profilo preesistente, riattiva (`attivo: true`, `nome`, `cognome`) se aggiunto con profilo disattivato, disattiva (`attivo: false`, `nome`, `cognome`, mai una delete) se tolto con profilo attivo, altrimenti — se il profilo resta invariato ma esiste già — sincronizza comunque `Collaboratore.nome`/`cognome` con l'anagrafica corrente; rivalida l'anagrafica collaboratori ogni volta che il profilo è stato toccato o risincronizzato. Il flusso inverso è simmetrico: `aggiornaCollaboratore` (capability Collaboratori) propaga `nome`/`cognome` all'`Utente` associato nella stessa transazione, invece di comporvi una stringa — vedi [Cognome nella sezione anagrafica dell'utente](/decisions/cognome-anagrafica-utente.md).
...
| Dati | `prisma/schema.prisma` e le migrazioni `20260721084945_aggiungi_stato_attivo_utente`, `20260728135453_aggiungi_cognome_utente` (`Utente`, `Account`; `Session` e `VerificationToken` dichiarati ma non usati dal flusso corrente) |
