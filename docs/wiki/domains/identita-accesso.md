...
summary: Accesso Google, sessione JWT, ruoli, policy di rotta e segregazione dei dati
status: generated
links:
  - id: architecture.context-map
    relation: participates-in
  - id: domains.collaboratori
    relation: consumes-operational-profile
sources:
...
3. `db.account.upsert` crea l'account Google o aggiorna l'access token; `createSession` scrive il cookie JWT e il callback reindirizza a `/attivita`.
4. `verificaERinnovaTokenSessione` assegna un nuovo `expiresAt = now + 8h` e il proxy riscrive il cookie sulle richieste previste. Non usa la tabella Prisma `Session`.
5. Logout e pulizia token impostano il cookie vuoto con `maxAge: 0`.
...
7. Non è osservata alcuna write di cambio ruolo. La creazione di un nuovo utente collaboratore assegna `COLLABORATORE`; un amministratore riusato mantiene il suo ruolo.
...
JWT firmato HS256, payload validato, `exp === expiresAt`, durata sliding 8 ore. `SESSION_SECRET` deve avere almeno 32 caratteri e non essere il placeholder. Cookie HttpOnly, SameSite=Lax, Secure in produzione. Google non autocrea utenti: richiede email verificata e già censita. `Utente.email`, account provider e profilo utente sono unici a database. Se i claim Google di verifica email sono entrambi assenti, l'adapter usa un fallback `true`. Il proxy non consulta il database: revoca del profilo e cambiamenti al ruolo diventano autorevoli quando il consumer invoca il DAL. `src/lib/auth.ts` è un placeholder; Auth.js non è una dipendenza runtime osservata.
